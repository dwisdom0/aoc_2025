...
def solve(data: list[tuple[int, int, int]]) -> int:
    ...
    pbar = tqdm(total=len(data), ascii=True)
    proposed_con = None
    num_circuits = len(set(id_to_circuit.values()))
    ...
    # look up the final vectors that we connected
    assert proposed_con is not None
    v0_x = data[proposed_con[0]][0]
    v1_x = data[proposed_con[1]][0]
    return v0_x * v1_x
